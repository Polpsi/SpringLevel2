...
create table users (id bigint not null, archive bit not null, email varchar(255), name varchar(255), password varchar(255), role varchar(255), bucket_id bigint, primary key (id)) engine=InnoDB;
alter table bucket_products add constraint bucket_products_fk_product foreign key (product_id) references products (id);
alter table bucket_products add constraint bucket_products_fk_bucket foreign key (bucket_id) references buckets (id);
alter table buckets add constraint buckets_fk_user foreign key (user_id) references users (id);
alter table orders add constraint orders_fk_user foreign key (user_id) references users (id);
alter table orders_details add constraint orders_details_fk_order foreign key (order_id) references orders (id);
alter table orders_details add constraint orders_details_fk_product foreign key (product_id) references products (id);
...
alter table users add constraint users_fk_bucket foreign key (bucket_id) references buckets (id);